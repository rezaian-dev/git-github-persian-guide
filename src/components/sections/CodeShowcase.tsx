...
const CHIPS = ["git switch", "Ctrl+Shift+G", "Graph", "Rulesets", "Actions", "Coding Agent"];

const LINES = [
  { text: "# شاخه را عوض کن — نه checkout دوچهره", cls: "text-faint" },
  { text: "git switch -c feature/rulesets", cls: "text-sky-300" },
  { text: "", cls: "" },
  { text: "# فقط فایل را برگردان؛ تاریخچه را لمس نکن", cls: "text-faint" },
  { text: "git restore --source=HEAD~1 -- src/app.py", cls: "text-sky-300" },
  { text: "", cls: "" },
  { text: "# وضعیت سه لایه را یک‌جا ببین", cls: "text-faint" },
  { text: "git status -sb", cls: "text-sky-300" },
];
          ...
          <div ref={ref} className="glass grid items-center gap-10 rounded-3xl p-6 md:grid-cols-[0.85fr_1.15fr] md:p-10">
            <div>
              ...
              <h2 className="text-[clamp(26px,3.5vw,38px)] font-extrabold leading-tight">
                ترمینال می‌فهمی؛ <span className="grad-text">VS Code</span> کار می‌کنی
              </h2>
              <p className="mt-3.5 text-[15px] leading-8 text-muted-foreground">
                هر مهارت دو شکل دارد. اگر UI تو با عکس یکی نبود، علت در همان صفحه نوشته شده است.
              </p>
            ...
            <div dir="ltr" className="overflow-hidden rounded-2xl border border-input bg-card shadow-[0_40px_110px_-30px_rgb(6_10_24_/_0.65)]">
              <div className="flex items-center gap-1.5 border-b border-border bg-secondary/40 px-4 py-3 font-mono text-[11.5px] text-muted-foreground">
                ...
                <span className="ms-2">~/projects/persian-guide — bash</span>
              </div>
              <pre className="min-h-[240px] whitespace-pre-wrap p-5 text-left font-mono text-[clamp(12.5px,1.4vw,14px)] leading-8 text-sub" tabIndex={0}>
                {done
                  ? LINES.map((l, idx) =>
                      l.text === "" ? "\n" : (
